...
export default function Card({ id, type, hp, status, team, isSelected }) {
  const { selectUnit } = useContext(ActionContext);
  ...
  return (
    <button
      onClick={!isKilled ? () => selectUnit({id, type, hp, status, team}) : undefined}
      disabled={isKilled}
      class={cardClasses}
    >
